...
static void Initialize(WebApplication app)
{
    ...
    StartCreateDecreaseTransactionCommandConsumer(kafkaService);

    StartReverseTransactionCommandConsumer(kafkaService);
}
...
static void StartCreateDecreaseTransactionCommandConsumer(IKafkaService kafkaService)
{
    kafkaService.StartConsumer<string, string>(new[]
    {
        nameof(CreateDecreaseTransactionCommand)
    }, autoCommit: true, async (result, consumer, provider) =>
    {
        ...
        var message = JsonSerializer.Deserialize<CreateDecreaseTransactionCommand>(result.Message.Value);
        ...
        try
        {
            ...
            await producer.ProduceAsync(nameof(CheckoutOrderTransactionEvent), new Confluent.Kafka.Message<string, string>
            {
                ...
                {
                    OrderId = order.Id,
                    Event = nameof(NewOrderTransactionEvent),
                    EventPayload = JsonSerializer.Serialize(new NewOrderTransactionEvent
                    {
                        OrderId = order.Id,
        ...
        catch (Exception ex)
        {
            ...
            await producer.ProduceAsync(nameof(CheckoutOrderTransactionEvent), new Confluent.Kafka.Message<string, string>
            {
                ...
                {
                    OrderId = order.Id,
                    Event = nameof(OrderAccountFailureEvent),
                    EventPayload = JsonSerializer.Serialize(new OrderAccountFailureEvent
                    {
                        OrderId = order.Id,
                        Reason = ex.Message,
                    })
...
static void StartReverseTransactionCommandConsumer(IKafkaService kafkaService)
{
    kafkaService.StartConsumer<string, string>(new[]
    {
        nameof(ReverseTransactionCommand)
    }, autoCommit: true, async (result, consumer, provider) =>
    {
        var dbContext = provider.GetRequiredService<AccountDbContext>();
        var accountService = provider.GetRequiredService<IAccountService>();
        var systemClient = provider.GetRequiredService<ISystemClient>();
        var message = JsonSerializer.Deserialize<ReverseTransactionCommand>(result.Message.Value);
