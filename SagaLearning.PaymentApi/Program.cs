...
static void Initialize(WebApplication app)
{
    ...
    StartMakePaymentCommandConsumer(kafkaService);

    StartFailPaymentCommandConsumer(kafkaService);
}

static void StartMakePaymentCommandConsumer(IKafkaService kafkaService)
{
    kafkaService.StartConsumer<string, string>(new[]
    {
        nameof(MakePaymentCommand)
    }, autoCommit: true, async (result, consumer, provider) =>
    {
        ...
        var message = JsonSerializer.Deserialize<MakePaymentCommand>(result.Message.Value);
        ...
        try
        {
            ...
            await producer.ProduceAsync(nameof(CheckoutOrderTransactionEvent), new Confluent.Kafka.Message<string, string>
            {
                ...
                Value = JsonSerializer.Serialize(new CheckoutOrderTransactionEvent
                {
                    ...
                    Event = nameof(OrderSuccessPaymentEvent),
                    EventPayload = JsonSerializer.Serialize(new OrderSuccessPaymentEvent
                    {
                        OrderId = message.OrderId
                    })
                })
        ...
        catch (Exception ex)
        {
            ...
            await producer.ProduceAsync(nameof(CheckoutOrderTransactionEvent), new Confluent.Kafka.Message<string, string>
            {
                ...
                {
                    OrderId = message.OrderId,
                    Event = nameof(OrderPaymentFailureEvent),
                    EventPayload = JsonSerializer.Serialize(new OrderPaymentFailureEvent
                    {
                        OrderId = message.OrderId,
                        Reason = ex.Message,
                    })
...
static void StartFailPaymentCommandConsumer(IKafkaService kafkaService)
{
    kafkaService.StartConsumer<string, string>(new[]
    {
    ...
    }, autoCommit: true, async (result, consumer, provider) =>
    {
        var paymentService = provider.GetRequiredService<IPaymentService>();
        var systemClient = provider.GetRequiredService<ISystemClient>();
        var message = JsonSerializer.Deserialize<FailPaymentCommand>(result.Message.Value);
